...
const CardList = () => {
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await fetch(`/api/enroll/delete?id=${id}`, {
        method: "DELETE",
      });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/api/enroll/get-all");
  ...
  const calculateProgress = (course) => {
    const totalChapters = course.course_id.chapters.length;
    const completedAssignments = course.assignments.length;
    return Math.round((completedAssignments / totalChapters) * 100);
  };
  return (
    <AdminLayout>
      ...
      <ToastContainer
        ...
        <div className="flex items-center justify-between mb-6 border p-1">
          <h2 className="text-2xl font-bold text-gray-800">Enrolled List</h2>
        </div>
        {loading && (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Link
                        ...
                        as={`/adminDashboard/user/${enroll.user_id.email}`}
                      >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Link
                        ...
                        as={`/adminDashboard/course/chapters/${enroll.course_id._id}`}
                      >
                        <div className="text-sm font-medium text-blue-500 cursor-pointer">
                          {enroll.course_id.title}
                        </div>{" "}
                      </Link>
                    </td>
                    <td className="px-6 py-4 ">
                      <div className="w-16 h-16 ">
                        <img
                          src={enroll.course_id.feature_img}
                          alt={enroll.course_id.title}
                          className="w-full h-full "
                        />
                      </div>
                    </td>
                    <td className="px-6 py-4 ">
                      {enroll.assignments.some((assignment) => assignment.marks !== null) ? (
                        <><div>
                          <span>Checked: </span>
                          <span>
                            {
                              enroll.assignments.filter(
                        ...
                        <span>Unchecked: {enroll.assignments.length}</span>
                      )}
                    </td>
                    <td className="px-6 py-4 ">
                      <input
                        ...
                        min="0"
                        max="100"
                        value={calculateProgress(enroll)}
                      />
                    </td>
                    <td className="px-6 py-4 ">
                      <Link
                        href={`/adminDashboard/enrolled/assignment-list/[id]`}
                        as={`/adminDashboard/enrolled/assignment-list/${enroll._id}`}
                      >
        ...
        {showConfirmation && (
          <ConfirmationModal
            ...
            text="Are you sure you want to delete this enrollement?"
          />
        )}
      </div>
    </AdminLayout>
